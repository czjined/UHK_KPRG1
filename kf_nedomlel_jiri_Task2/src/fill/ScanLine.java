    ...
    private List<Line> prepareLines(List<Line> lineList) {
        ...
        for (Line line : lineList) {
            if (line.getY1() != line.getY2()) {                 // Neberu vodorovne hrany
                ...
                if (preparedLines.get(preparedLines.size() - 1).getY2() > ymax) {
                    ymax = preparedLines.get(preparedLines.size() - 1).getY2();
                }
                if (preparedLines.get(preparedLines.size() - 1).getY1() < ymin) {
                    ymin = preparedLines.get(preparedLines.size() - 1).getY1();
                }
            }
        }
...
//        for (Point bod: pruseciky) {
//            System.out.println(String.format("Vypisuji (serazene) pruseciky: x = %d, y = %d ", bod.getX(), bod.getY()));
//        }
